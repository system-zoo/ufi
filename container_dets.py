...
def display_ip(args):
    detail = c.inspect_container(args.container)
    return detail['NetworkSettings']['IPAddress']
...
def display_all(args):
    containers = c.containers(filters={"label":"systemzoo"})
    container_dets = {}

    for container in containers: 
	    args.container = container["Id"]
	    status = {}
	    
	    status["containername"] = container["Names"][0][1:]
	    args.type = "system"
	    status["cpu_system"] = display_cpu(args)

	    args.type = "user"
	    status["cpu_user"] = display_cpu(args)

	    args.type = "all"
	    status["cpu_all"] = display_cpu(args)

	    status["ip"] = display_ip(args)
	    status["memory"] = display_memory(args)
	    
	    args.direction = "in"
	    status["newtork_in"] = display_ip(args)

	    args.direction = "out"
	    status["newtork_out"] = display_ip(args)
	    
	    args.direction = "status"
	    status["status"] = display_ip(args)
            container_dets[container["Id"]] = status

    print(json.dumps(container_dets))
...
all_parser = subparsers.add_parser("all",help="Display all the stats")
all_parser.set_defaults(func=display_all)
